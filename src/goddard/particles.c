...
void func_80181EB0(struct Connection *cxn) {
    ...
    sp0->unk38.y += sp34.y;
    sp0->unk38.z += sp34.z;
    if (!(sp4->flags & PTC_FLAG_2)) {
        sp4->pos.x -= sp34.x;
        sp4->pos.y -= sp34.y;
        sp4->pos.z -= sp34.z;
    }
    if (!(sp0->flags & PTC_FLAG_2)) {
        sp0->pos.x += sp34.x;
        sp0->pos.y += sp34.y;
        sp0->pos.z += sp34.z;
    }
...
void func_80182088(struct Connection *cxn) {
    ...
    sp18->unk38.y += sp4C.y;
    sp18->unk38.z += sp4C.z;
    if (!(sp1C->flags & PTC_FLAG_2)) {
        sp1C->pos.x -= sp4C.x;
        sp1C->pos.y -= sp4C.y;
        sp1C->pos.z -= sp4C.z;
    }
    if (!(sp18->flags & PTC_FLAG_2)) {
        sp18->pos.x += sp4C.x;
        sp18->pos.y += sp4C.y;
        sp18->pos.z += sp4C.z;
    }
            ...
            if (ptc->unk60 == 3) {
                switch (ptc->unk64) {
                    case 1:
                        ...
                        for (i = 0; i < 50; i++) {
                            sp60 = make_particle(PTC_FLAG_1, -1, ptc->pos.x, ptc->pos.y, ptc->pos.z);
                            sp60->shapePtr = ptc->shapePtr;
                            addto_group(ptc->subParticlesGrp, &sp60->header);
                            sp60->flags &= ~PTC_FLAG_8;
                        }
                        break;
                    case 2:
                    case 3:
