...
#define AUDIO_ALIGN(val, amnt) ALIGN((val), (1 << amnt))
                ...
                if (t0 != 0) {
                    ...
                    if (audioBookSample->medium == 0) {
                        ...
                        v0_2 = dma_sample_data((uintptr_t)(sp84 + (temp * unk_s6) + sampleAddr),
                                AUDIO_ALIGN(t0 * unk_s6 + 16, 4), flags, &synthesisState->sampleDmaIndex, audioBookSample->medium);
                    }

                    a3 = ((uintptr_t)v0_2 & 0xf);
                    aligned = AUDIO_ALIGN(t0 * unk_s6 + 16, 4);
                    addr = (DMEM_ADDR_COMPRESSED_ADPCM_DATA - aligned) & 0xffff;
                    aLoadBuffer(cmd++, VIRTUAL_TO_PHYSICAL2(v0_2 - a3), addr, AUDIO_ALIGN(t0 * unk_s6 + 16, 4));
                } else {
                    s0 = 0;
                    a3 = 0;
                }
                ...
                case 2:
                    switch (curPart) {
                        case 0:
                            ...
                            break;
                        case 1:
                            aDownsampleHalf(cmd++, AUDIO_ALIGN(samplesLenAdjusted / 2, 3), DMEM_ADDR_UNCOMPRESSED_NOTE + sp130, resampledTempLen + DMEM_ADDR_RESAMPLED);
                            break;
                    }
            }
            if (noteSubEu->finished) {
...
u64 *note_apply_headset_pan_effects(u64 *cmd, struct NoteSubEu *noteSubEu, struct NoteSynthesisState *note, s32 bufLen, s32 flags, s32 leftRight) {
    ...
    if (flags != 1) { // A_INIT?
        ...
        if (prevPanShift != 0) {
            aLoadBuffer(cmd++, VIRTUAL_TO_PHYSICAL2(note->synthesisBuffers->panSamplesBuffer),
                        DMEM_ADDR_NOTE_PAN_TEMP, AUDIO_ALIGN(prevPanShift, 4));
    ...
    if (panShift) {
        ...
        aSaveBuffer(cmd++, DMEM_ADDR_NOTE_PAN_TEMP + bufLen,
                    VIRTUAL_TO_PHYSICAL2(note->synthesisBuffers->panSamplesBuffer), AUDIO_ALIGN(panShift, 4));
    }
