...
void seq_channel_layer_process_script(struct SequenceChannelLayer *layer) {
    ...
    if (cmd != -1) {
        ...
        if (layer->stopSomething && (layer->note != NULL || layer->continuousNotes)) {
            seq_channel_layer_note_decay(layer);
        }
    }
}
...
void seq_channel_layer_process_script(struct SequenceChannelLayer *layer) {
    ...
    if (cmd == layer_delay) { // layer_delay
        ...
        if (seqChannel->largeNotes) {
            ...
            cmd -= (cmd & 0xc0);
 #if defined(VERSION_EU) || defined(VERSION_SH)
            layer->velocitySquare = sqr((f32) vel);
 #else
            layer->velocitySquare = sqr(vel);
 #endif
        } else {
            switch (cmd & 0xc0) {
    ...
    for (;;) {
        ...
        switch (cmd) {
            ...
            case layer_setpan: // layer_setpan
                temp_a0_5 = *(state->pc++);
                if (cmd == layer_setshortnotevelocity) {
                    layer->velocitySquare = (f32) sqr(temp_a0_5) / (f32) sqr(127);
                } else {
                    layer->pan = temp_a0_5;
                }
                break;
            ...
            default:
                switch (cmd & 0xf0) {
                    case layer_setshortnotevelocityfromtable: // layer_setshortnotevelocityfromtable
                        sp3A = seqPlayer->shortNoteVelocityTable[cmd & 0x0f];
                        layer->velocitySquare = (f32) sqr(sp3A) / (f32) sqr(127);
                        break;
                    case layer_setshortnotedurationfromtable: // layer_setshortnotedurationfromtable
                        layer->noteDuration = seqPlayer->shortNoteDurationTable[cmd & 0x0f];
                        break;
...
s32 seq_channel_layer_process_script_part3(struct SequenceChannelLayer *layer, s32 cmd) {
    ...
    if (seqChannel->largeNotes) {
        ...
        layer->velocitySquare = sqr((f32) vel) / (f32) sqr(0x7f);
        // the remaining bits are used for the semitone
        cmd -= (cmd & 0xc0);
    } else {
        switch (cmd & 0xc0) {
...
void sequence_channel_process_script(struct SequenceChannel *seqChannel) {
    ...
    if (seqChannel->delay == 0) {
        for (;;) {
            ...
            if (cmd == chan_delay) { // chan_delay
                seqChannel->delay = m64_read_compressed_u16(state);
                break;
            }
            if (cmd == chan_hang) { // chan_hang
...
#ifdef VERSION_SH
            if (cmd >= chan_unk_0xb0_threshold)
#else
            if (cmd > chan_unk_0xc0_threshold)
#endif
            {
                switch (cmd) {
                    case chan_end: // chan_end
#if defined(VERSION_EU) || defined(VERSION_SH)
                        if (state->depth == 0) {
                            sequence_channel_disable(seqChannel);
                            goto out;
...
#if defined(VERSION_EU) || defined(VERSION_SH)
                    case chan_delay1: // chan_delay1
                        goto out;

                    case chan_delay: // chan_delay
                        seqChannel->delay = m64_read_compressed_u16(state);
                        goto out;

                    case chan_unk_0xea_stop:
                        seqChannel->stopScript = TRUE;
                        goto out;
#endif
                    case chan_call: // chan_call
                        if (0 && state->depth >= 4) {
                    ...
                    case chan_loop: // chan_loop; loop start, N iterations (or 256 if N = 0)
                        if (0 && state->depth >= 4) {
                    ...
                    case chan_break: // chan_break; break loop, if combined with jump
                        state->depth--;
                        break;

                    case chan_jump: // chan_jump
                    case chan_beqz: // chan_beqz
                    case chan_bltz: // chan_bltz
                    case chan_bgez: // chan_bgez
                        sp5A = m64_read_s16(state);
                        if ((cmd == chan_beqz && value != 0)
                         || (cmd == chan_bltz && value >= 0)
                         || (cmd == chan_bgez && value <  0)) {
                            break;
                        }
                        state->pc = seqPlayer->seqData + sp5A;
                        break;
...
#if defined(VERSION_EU) || defined(VERSION_SH)
                    case chan_jump_rel: // chan_jump_rel
                    case chan_beqz_rel: // chan_beqz_rel
                    case chan_bltz_rel: // chan_bltz_rel
                        temp = m64_read_u8(state);
                        if ((cmd == chan_beqz_rel && value != 0)
                         || (cmd == chan_bltz_rel && value >= 0)) {
                            break;
                        }
                        state->pc += temp;
                        break;
                    ...
                    case chan_reservenotes: // chan_reservenotes
                        note_pool_clear(&seqChannel->notePool);
                        note_pool_fill(&seqChannel->notePool, m64_read_u8(state));
                        break;

                    case chan_unreservenotes: // chan_unreservenotes
                        note_pool_clear(&seqChannel->notePool);
                        break;

                    case chan_setdyntable: // chan_setdyntable
                        sp5A = m64_read_s16(state);
...
#if defined(VERSION_EU) || defined(VERSION_SH)
                    case chan_setbankandinstr: // chan_setbankandinstr
                        cmd = m64_read_u8(state);
                    ...
                    case chan_setinstr: // chan_setinstr ("set program"?)
                        set_instrument(seqChannel, m64_read_u8(state));
                        break;

                    case chan_largenotesoff: // chan_largenotesoff
                        seqChannel->largeNotes = FALSE;
                        break;

                    case chan_largenoteson: // chan_largenoteson
                        seqChannel->largeNotes = TRUE;
                        break;

                    case chan_setvol: // chan_setvol
                    ...
                    case chan_pitchbend: // chan_pitchbend; pitch bend by <= 1 octave in either direction (-127..127)
...
#ifdef VERSION_SH
                    case chan_unk_0xee_set_freqscale:
                        cmd = m64_read_u8(state) + 0x80;
                    ...
                    case chan_setpanmix: // chan_setpanmix; set proportion of pan to come from channel (0..128)
#if defined(VERSION_EU) || defined(VERSION_SH)
                    ...
                    case chan_setvibratodelay: // chan_setvibratodelay
                        seqChannel->vibratoDelay = m64_read_u8(state) * 16;
                        break;

#if defined(VERSION_JP) || defined(VERSION_US)
                    case chan_setupdatesperframe_unimplemented: // chan_setupdatesperframe_unimplemented
                        cmd = m64_read_u8(state);
                        if (cmd == 0) {
                    ...
                    case chan_setreverb: // chan_setreverb
                        seqChannel->reverbVol = m64_read_u8(state);
                        break;

                    case chan_setbank: // chan_setbank; switch bank within set
                        cmd = m64_read_u8(state);
                    ...
                    case chan_writeseq: // chan_writeseq; write to sequence data (!)
                        {
#if !defined(VERSION_EU) && !defined(VERSION_SH)
                            u8 *seqData;
#endif
                    ...
                    case chan_subtract: // chan_subtract
                    case chan_bitand: // chan_bitand
                    case chan_setval: // chan_setval
                        temp = m64_read_u8(state);
                        if (cmd == chan_subtract) {
                            value -= temp;
                        } else if (cmd == chan_setval) {
                            value = temp;
                        } else {
                            value &= temp;
                        }
                    ...
                    case chan_setmutebhv: // chan_setmutebhv
                    ...
                    case chan_stereoheadseteffects: // chan_stereoheadseteffects
                        seqChannel->stereoHeadsetEffects = m64_read_u8(state);
                        break;

                    case chan_setnoteallocationpolicy: // chan_setnoteallocationpolicy
                        seqChannel->noteAllocPolicy = m64_read_u8(state);
                        break;

                    case chan_setsustain: // chan_setsustain
#if defined(VERSION_EU) || defined(VERSION_SH)
                        ...
                        break;
#if defined(VERSION_EU) || defined(VERSION_SH)
                    case chan_setreverbindex:
                        seqChannel->reverbIndex = m64_read_u8(state);
                        break;
#endif
                    case chan_dyncall: // chan_dyncall
                        if (value != -1) {
#if defined(VERSION_EU) || defined(VERSION_SH)
                            if (state->depth >= 4) {
...
#endif // (VERSION_EU || VERSION_SH)
#ifdef VERSION_SH
                    case chan_setsynthesisvolume:
                        seqChannel->synthesisVolume = m64_read_u8(state);
                        break;

                    case chan_unk_0xef_read:
                        m64_read_s16(state);
                        m64_read_u8(state);
                        break;

                    case chan_unk_0xb0_set_filter:
                        sp5A = m64_read_s16(state);
                        seqData = seqPlayer->seqData + sp5A;
                        seqChannel->filter = (s16 *) (seqData);
                        break;

                    case chan_unk_0xb1_null_filter:
                        seqChannel->filter = NULL;
                        break;

                    case chan_unk_0xb3_fill_filter:
                    ...
                    case chan_unk_0xb4_set_dyntable:
                ...
                if (cmd >= 0x80) {
                    ...
                    switch (cmd & 0xf8) {
                        ...
                        case chan_unk_0x88_set_scriptstate_pc:
                            sp5A = m64_read_s16(state);
                ...
                switch (cmd & 0xf0) {
#ifdef VERSION_SH
                    case chan_unk_0x00_set_delay:
                        seqChannel->delay = loBits;
                        goto out;

                    case chan_unk_0x10_set_soundscriptio:
                    ...
                    // sh: 0x70
                    case chan_iowriteval: // chan_iowriteval; write data back to audio lib
                        seqChannel->soundScriptIO[loBits] = value;
                        break;

                    case chan_ioreadval: // chan_ioreadval; read data from audio lib
 ...
 #ifdef VERSION_EU
                    // sh: 0x00
                    case chan_delayshort: // chan_delayshort
                        seqChannel->delay = loBits;
                        goto out;
 #endif // VERSION_EU

                    case chan_setlayer: // chan_setlayer
                        sp5A = m64_read_s16(state);
                        if (seq_channel_set_layer(seqChannel, loBits) == 0) {
                            seqChannel->layers[loBits]->scriptState.pc = seqPlayer->seqData + sp5A;
                        }
                    ...
                    case chan_freelayer: // chan_freelayer
                        seq_channel_layer_free(seqChannel, loBits);
                        break;
 ...
 #ifndef VERSION_EU
                    case chan_setnotepriority: // chan_setnotepriority (arg must be >= 2)
                        seqChannel->notePriority = loBits;
                        break;
 #endif // !VERSION_EU
#endif // !VERSION_SH

                    case chan_startchannel: // chan_startchannel
                        sp5A = m64_read_s16(state);
                        sequence_channel_enable(seqPlayer, loBits, seqPlayer->seqData + sp5A);
                        break;

#ifndef VERSION_SH
                    case chan_disablechannel: // chan_disablechannel
                        sequence_channel_disable(seqPlayer->channels[loBits]);
                        break;
#endif

                    case chan_iowriteval2: // chan_iowriteval2; write data back to audio lib for another channel
                        cmd = m64_read_u8(state);
                        seqPlayer->channels[loBits]->soundScriptIO[cmd] = value;
                        break;

                    case chan_ioreadval2: // chan_ioreadval2; read data from audio lib from another channel
                        cmd = m64_read_u8(state);
...
void sequence_player_process_sequence(struct SequencePlayer *seqPlayer) {
    ...
    if (seqPlayer->delay > 1) {
        ...
        for (;;) {
            ...
            if (cmd >= 0xc0) {
                switch (cmd) {
                    case seq_end: // seq_end
                        break;

                    case seq_call: // seq_call
                    ...
                    case seq_loop: // seq_loop; loop start, N iterations (or 256 if N = 0)
                        if (0 && state->depth >= 4) {
                            eu_stubbed_printf_0("Macro Level Over Error!\n");
                        }
                        state->remLoopIters[state->depth] = m64_read_u8(state);
                    ...
                    case seq_jump: // seq_jump
                    case seq_beqz: // seq_beqz; jump if == 0
                    case seq_bltz: // seq_bltz; jump if < 0
                    case seq_bgez: // seq_bgez; jump if >= 0
                        u16v = m64_read_s16(state);
                        if ((cmd == seq_beqz && value != 0)
                         || (cmd == seq_bltz && value >= 0)
                         || (cmd == seq_bgez && value <  0)) {
                            break;
                        }
                        state->pc = seqPlayer->seqData + u16v;
                        break;
                    ...
                    case seq_unk_f2:
                        temp = m64_read_u8(state);
                        if ((cmd == seq_unk_f3 && value != 0)
                         || (cmd == seq_unk_f2 && value >= 0)) {
                            break;
                        }
                        state->pc += (s8) temp;
                        break;
                    ...
                    case seq_reservenotes: // seq_reservenotes
                        note_pool_clear(&seqPlayer->notePool);
                        note_pool_fill(&seqPlayer->notePool, m64_read_u8(state));
                        break;

                    case seq_unreservenotes: // seq_unreservenotes
                        note_pool_clear(&seqPlayer->notePool);
                        break;
                    ...
                    case seq_settempo: // seq_settempo (bpm)
#ifdef VERSION_SH
                        seqPlayer->tempo = m64_read_u8(state) * TEMPO_SCALE;
#else
                    case seq_addtempo: // seq_addtempo (bpm)
                        temp = m64_read_u8(state);
                        if (cmd == seq_settempo) {
                    ...
                    case seq_initchannels: // seq_initchannels
                        u16v = m64_read_s16(state);
                        sequence_player_init_channels(seqPlayer, u16v);
                        break;

                    case seq_disablechannels: // seq_disablechannels
                        u16v = m64_read_s16(state);
                    ...
                    case seq_setmutebhv: // seq_setmutebhv
                        seqPlayer->muteBehavior = m64_read_u8(state);
                        break;

                    case seq_setshortnotevelocitytable: // seq_setshortnotevelocitytable
                    case seq_setshortnotedurationtable: // seq_setshortnotedurationtable
                        u16v = m64_read_s16(state);
                        seqData = seqPlayer->seqData + u16v;
                        if (cmd == seq_setshortnotevelocitytable) {
                    ...
                    case seq_setnoteallocationpolicy: // seq_setnoteallocationpolicy
                        seqPlayer->noteAllocPolicy = m64_read_u8(state);
                        break;

                    case seq_setval: // seq_setval
                        value = m64_read_u8(state);
                        break;

                    case seq_bitand: // seq_bitand
#if defined(VERSION_EU) || defined(VERSION_SH)
                        value &= m64_read_u8(state);
#else
                    ...
                    case seq_subtract: // seq_subtract
                ...
                switch (cmd & 0xf0) {
                    ...
                    case seq_unk_4X:
                        break;
                    case seq_subvariation: // seq_subvariation
#if defined(VERSION_EU) || defined(VERSION_SH)
                    ...
                    case seq_unk_6X:
                        break;
                    case seq_setvariation: // seq_setvariation
#if defined(VERSION_EU) || defined(VERSION_SH)
...
#endif
                        break;
                    case seq_getvariation: // seq_getvariation
#if defined(VERSION_EU) || defined(VERSION_SH)
                        value = seqPlayer->seqVariationEu[0];
#else
                        value = seqPlayer->seqVariation;
#endif
                        break;
                    case seq_startchannel: // seq_startchannel
                        u16v = m64_read_s16(state);
                    ...
                    case seq_unk_aX:
                        break;
#if !defined(VERSION_EU) && !defined(VERSION_SH)
                    case seq_unk_d8: // (this makes no sense)
                        break;
                    case seq_unk_d9:
                        break;
#endif
