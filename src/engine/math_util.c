...
static void find_floor_at_relative_angle(Vec3f point, Vec3f pos, s16 yaw, s16 angle, f32 radius, f32 height, f32 minY) {
    s16 dir = yaw + angle;
...
f32 find_surface_on_ray(Vec3f orig, Vec3f dir, struct Surface** hit_surface, Vec3f hit_pos, s32 flags) {
    ...
    *hit_surface = NULL;
    ...
    f32 t_max_x = absf((p_x + MAX(stp_x, 0.0f) - start_cell_coord_x) * rdinv_x);
    f32 t_max_z = absf((p_z + MAX(stp_z, 0.0f) - start_cell_coord_z) * rdinv_z);
...
f32 find_surface_on_ray_between_points(Vec3f pos1, Vec3f pos2, struct Surface** hit_surface, Vec3f hit_pos, s32 flags) {
    ...
    return find_surface_on_ray(pos1, dir, hit_surface, hit_pos, flags);
}
