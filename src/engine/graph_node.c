...
void geo_call_global_function_nodes_helper(struct GraphNode *graphNode, s32 callContext) {
    ...
    do {
        ...
        type = curNode->type;
        // These types previously had GRAPH_NODE_TYPE_FUNCTIONAL, which was checked here.
        // However, the previous method caused very large enum values, which messed with the
        // jump table generated by the switch statement in geo_process_node_and_siblings.
...
void geo_obj_init(struct GraphNodeObject *graphNode, void *sharedChild, Vec3f pos, Vec3s angle) {
    vec3_same(graphNode->scale, construct_float(1.0f));
    vec3f_copy(graphNode->pos, pos);
    vec3s_copy(graphNode->angle, angle);
