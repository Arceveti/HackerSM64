...
static void level_cmd_load_model_from_dl(void) {
    ...
    if (model < MODEL_ID_COUNT) {
        gLoadedGraphNodes[model] = (struct GraphNode*)init_graph_node_display_list(sLevelPool, 0, layer, dl_ptr);
    }
...
#endif
        gAreas[sCurrAreaIndex].musicSeqId = CMD_GET(s16, 6); // music/seq
    }
    sCurrentCmd = CMD_NEXT;
}
...
static void level_cmd_fadeout_music(void) {
    ...
    if (sCurrAreaIndex != -1 && dur == 0) {
        ...
        gAreas[sCurrAreaIndex].musicSettingsPreset = 0x00;
        gAreas[sCurrAreaIndex].musicSeqId = 0x00;
#ifdef BETTER_REVERB
...
static void level_cmd_puppylight_node(void) {
    ...
    vec4_set(gPuppyLights[gNumLights]->rgba,
        CMD_GET(u8,   2),
        CMD_GET(u8,   3),
        CMD_GET(u8,   4),
        CMD_GET(u8,   5)
    );
    vec3s_set(gPuppyLights[gNumLights]->pos[0],
        CMD_GET(s16,  6),
        CMD_GET(s16,  8),
        CMD_GET(s16, 10)
    );
    vec3s_set(gPuppyLights[gNumLights]->pos[1],
        CMD_GET(s16, 12),
        CMD_GET(s16, 14),
        CMD_GET(s16, 16)
    );
