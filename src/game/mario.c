...
u32 mario_get_terrain_sound_addend(struct MarioState* m) {
    ...
    if (m->floor != NULL) {
        ...
        if ((gCurrLevelNum != LEVEL_LLL) && (m->floorHeight < (m->waterLevel - 10))) {
            ...
            switch (floorType) {
                default:
                    floorSoundType = FLOOR_SOUND_TYPE_DEFAULT;
                    break;
                ...
                case SURFACE_HARD_NOT_SLIPPERY:
                case SURFACE_SWITCH:
                    floorSoundType = FLOOR_SOUND_TYPE_HARD;
                    break;
                ...
                case SURFACE_NOISE_VERY_SLIPPERY:
                case SURFACE_NO_CAM_COL_VERY_SLIPPERY:
                    floorSoundType = FLOOR_SOUND_TYPE_VERY_SLIPPERY;
                    break;

                case SURFACE_NOISE_DEFAULT:
                    floorSoundType = FLOOR_SOUND_TYPE_NOISE_DEFAULT;
                    break;

                case SURFACE_NOISE_SLIPPERY:
                    floorSoundType = FLOOR_SOUND_TYPE_NOISE_SLIPPERY;
                    break;
            }
...
f32 find_floor_height_relative_polar(struct MarioState* m, s16 angleFromMario, f32 distFromMario) {
    ...
    f32 x = sins(m->faceAngle[1] + angleFromMario) * distFromMario;
    f32 z = coss(m->faceAngle[1] + angleFromMario) * distFromMario;

    return find_floor(m->pos[0] + x, m->pos[1] + 100.0f, m->pos[2] + z, &floor);
}
...
s32 find_floor_slope(struct MarioState* m, s16 yawOffset) {
    ...
    if (absf(m->moveSpeed) > FAST_FLOOR_ALIGN) {
...
void debug_print_speed_action_normal(struct MarioState* m) {
    ...
    if (gShowDebugText) {
        ...
        print_text_fmt_int(210, 88, "ANG %d", angle_to_degrees(atan2s(floor_nY, steepness)));
...
void update_mario_joystick_inputs(struct MarioState* m) {
    ...
    f32 mag = sqr(controller->stickMag) / 64.0f;
...
void mario_reset_bodystate(struct MarioState* m) {
    ...
    bodyState->modelState = MODEL_STATE_NORMAL;
...
void mario_update_hitbox_and_cap_model(struct MarioState* m) {
    ...
    if (m->action & ACT_FLAG_SHORT_HITBOX) {
        m->marioObj->hitboxHeight = MARIO_SHORT_HITBOX_HEIGHT;
    } else {
...
void init_mario(struct MarioState* m) {
    ...
    if (save_file_get_flags()
        & (SAVE_FLAG_CAP_ON_GROUND | SAVE_FLAG_CAP_ON_KLEPTO | SAVE_FLAG_CAP_ON_UKIKI | SAVE_FLAG_CAP_ON_MR_BLIZZARD)
    ) {
        m->flags = MARIO_NONE;
    } else {
