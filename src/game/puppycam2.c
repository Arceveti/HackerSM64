...
static void puppycam_analogue_stick(void) {
    ...
    if (absi(gPuppyCam.stick2[0]) < DEADZONE) {
        gPuppyCam.stick2[0] = 0;
        gPuppyCam.stickN[0] = 0;
    }
    if (absi(gPuppyCam.stick2[1]) < DEADZONE) {
...
void puppycam_boot(void) {
    ...
    gPuppyCam.povHeight     = MARIO_EYE_HEIGHT;
    gPuppyCam.stick2[0]     = 0;
    gPuppyCam.stick2[1]     = 0;
    gPuppyCam.stickN[0]     = 0;
    gPuppyCam.stickN[1]     = 0;
...
void puppycam_display_options() {
    ...
    puppycam_display_box( 48,  84, 272, 218, 0x00, 0x00, 0x00, 0x50);
...
static void puppycam_input_hold_preset3(f32 ivX) {
    ...
    if (
        (
            ((gPlayer1Controller->buttonPressed & D_CBUTTONS) && !gPuppyCam.options.analogue) ||
            (gPuppyCam.stick2[1] < -DEADZONE && !gPuppyCam.stickN[1]) ||
            (gPlayer1Controller->buttonPressed & (A_BUTTON | B_BUTTON))
        ) &&
        gPlayer1Controller->stickMag < DEADZONE
    ) {
        if (gPuppyCam.mode3Flags & PUPPYCAM_MODE3_ZOOMED_IN) {
            gPuppyCam.stickN[1] = 1;
            gPuppyCam.mode3Flags |= PUPPYCAM_MODE3_ZOOMED_MED;
...
void puppycam_wall_angle(void) {
    ...
    s16 wallYaw = get_surface_yaw(wall) + 0x4000;
...
void puppycam_projection_behaviours(void) {
    ...
    if (gPuppyCam.targetObj == gMarioState->marioObj) {
        if (
            (
                (gPuppyCam.options.turnHelper || gPuppyCam.flags & PUPPYCAM_BEHAVIOUR_TURN_HELPER) &&
            ...
            if (gPuppyCam.options.turnHelper || gMarioState->action & ACT_FLAG_BUTT_OR_STOMACH_SLIDE || gMarioState->action == ACT_SHOT_FROM_CANNON || gMarioState->action == ACT_FLYING) {
                ...
                if (
                    (abss(gPlayer1Controller->rawStickX) > 20 && !(gMarioState->action & ACT_FLAG_BUTT_OR_STOMACH_SLIDE)) ||
                    (gMarioState->action == ACT_SHOT_FROM_CANNON) ||
                    (
                        (gMarioState->action & ACT_FLAG_BUTT_OR_STOMACH_SLIDE) &&
                        ((s16)absi((((gPuppyCam.yaw + 0x8000) % 0xFFFF) - 0x8000) - (((gMarioState->faceAngle[1]) % 0xFFFF) - 0x8000)) < 0x3000)
                    )
                ) {
        ...
        gPuppyCam.lastTargetFloorHeight = approach_f32_asymptotic(gPuppyCam.lastTargetFloorHeight, gPuppyCam.targetFloorHeight
                                                                ...
                                                                + CLAMP(((absf(gPuppyCam.targetFloorHeight - gPuppyCam.lastTargetFloorHeight)) / 300.0f), 0, 0.1f));
        ...
        if ((gMarioState->action & (ACT_FLAG_MOVING | ACT_FLAG_AIR | ACT_FLAG_ON_POLE)) || ((gMarioState->action & ACT_FLAG_SWIMMING) && !(((gMarioState->waterLevel - 100) - gMarioState->pos[1]) > 5) && gMarioState->forwardVel != 0.0f)) {
...
static void puppycam_projection(void) {
    ...
    if (gPuppyCam.targetObj) {
        vec3f_to_vec3s(targetPos, &gPuppyCam.targetObj->oPosVec);
        vec3s_copy(targetPos3, targetPos);
        if (gPuppyCam.targetObj2) {
            vec3f_to_vec3s(targetPos2, &gPuppyCam.targetObj2->oPosVec);
        ...
        if (gMarioState->action == ACT_DEBUG_FREE_MOVE) {
            gPuppyCam.targetDist[1] = MAX((gPuppyCam.targetDist[1] * 1.1f), 500);
        }
#endif
...
#ifdef ENABLE_DEBUG_FREE_MOVE
        if (gMarioState->action == ACT_DEBUG_FREE_MOVE) {
            vec3_copy_y_off(gPuppyCam.focus, gMarioState->pos, 160.0f);
        }
#endif
...
static void puppycam_collision(void) {
    ...
    dist[0] = find_surface_on_ray(target[0], vecToCam, &surf[0], hitpos[0], RAYCAST_FIND_SOLID);
    dist[1] = find_surface_on_ray(target[1], vecToCam, &surf[1], hitpos[1], RAYCAST_FIND_SOLID);
