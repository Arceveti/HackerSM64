...
void puppycam_default_config(void) {
    gPuppyCam.options.invertX        = TRUE;
    gPuppyCam.options.invertY        = TRUE;
    ...
    gPuppyCam.options.inputType      = PUPPYCAM_INPUT_TYPE_SINGLE_PRESS;
}
...
static void puppycam_collision(void) {
    ...
    vec3f_copy_y_off(target[0], &gPuppyCam.targetObj->oPosVec, (gPuppyCam.povHeight - CLAMP(gPuppyCam.targetObj->oPosY - gPuppyCam.targetFloorHeight, 0, 300)));
