...
void puppylights_run(Lights1 *src, struct Object *obj, s32 flags, u32 baseColour) {
    ...
    for (i = 0; i < gNumLights; i++) {
        if (gPuppyLights[i]->rgba[3] > 0 && gPuppyLights[i]->active == TRUE && gPuppyLights[i]->area == gCurrAreaIndex && (gPuppyLights[i]->room == ROOM_NULL || gPuppyLights[i]->room == gMarioCurrentRoom)) {
