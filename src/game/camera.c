...
void calc_y_to_curr_floor(f32 *posOff, f32 posMul, f32 posBound, f32 *focOff, f32 focMul, f32 focBound) {
    ...
    *posOff = CLAMP(*posOff, -posBound, posBound);

    *focOff = (floorHeight - sMarioCamState->pos[1]) * focMul;

    *focOff = CLAMP(*focOff, -focBound, focBound);
}
...
void radial_camera_move(struct Camera *c) {
    ...
    if (avoidStatus == AVOID_STATUS_WALL_COVERING_MARIO) {
        ...
        avoidYaw = CLAMP(avoidYaw, DEGREES(-105), DEGREES(105));
    }
    ...
    sModeOffsetYaw = CLAMP(sModeOffsetYaw, DEGREES(-120), DEGREES(120));
}
...
s32 update_parallel_tracking_camera(struct Camera *c, Vec3f focus, Vec3f pos) {
    ...
    vec3f_diff(marioOffset, marioPos, parMidPoint);
...
s32 update_fixed_camera(struct Camera *c, Vec3f focus, UNUSED Vec3f pos) {
    ...
    focus[1] += focusFloorOff + MARIO_EYE_HEIGHT;
    vec3f_get_dist_and_angle(focus, c->pos, &distCamToFocus, &faceAngle[0], &faceAngle[1]);
    faceAngle[2] = 0;

    vec3f_sum(basePos, sFixedModeBasePosition, sCastleEntranceOffset);
...
s32 update_boss_fight_camera(struct Camera *c, Vec3f focus, Vec3f pos) {
    ...
    focus[1] = (sMarioCamState->pos[1] + secondFocus[1]) / 2.f + MARIO_EYE_HEIGHT;
...
s32 mode_behind_mario(struct Camera *c) {
    ...
    gCameraMovementFlags = COND_BIT(gCameraMovementFlags, CAM_MOVE_SUBMERGED, (c->pos[1] <= waterHeight));
 ...
 */
s32 update_slide_camera(struct Camera *c) {
    struct Surface *floor;
    f32 floorHeight;
    Vec3f pos;
    f32 distCamToFocus;
    ...
    // Slide mode
        ...
        floorHeight = find_floor(c->pos[0], c->pos[1] + 200.f, c->pos[2], &floor) + MARIO_EYE_HEIGHT;
...
s32 update_mario_camera(UNUSED struct Camera *c, Vec3f focus, Vec3f pos) {
    ...
    focus_on_mario(focus, pos, MARIO_EYE_HEIGHT, MARIO_EYE_HEIGHT, gCameraZoomDist, DEGREES(8), yaw);
 ...
 */
s32 update_default_camera(struct Camera *c) {
    Vec3f tempPos;
    Vec3f cPos;
    ...
    cPos[1] += posHeight + MARIO_EYE_HEIGHT;

    // Move the camera away from walls and set the collision flag
    if (collide_with_walls(cPos, 10.0f, 80.0f) != 0) {
        sStatusFlags |= CAM_FLAG_COLLIDED_WITH_WALL;
    }

    c->focus[0] = sMarioCamState->pos[0];
    c->focus[1] = sMarioCamState->pos[1] + MARIO_EYE_HEIGHT + focHeight;
    c->focus[2] = sMarioCamState->pos[2];

    marioFloorHeight = MARIO_EYE_HEIGHT + sMarioGeometry.currFloorHeight;
    marioFloor = sMarioGeometry.currFloor;
    camFloorHeight = find_floor(cPos[0], cPos[1] + 50.f, cPos[2], &cFloor) + MARIO_EYE_HEIGHT;
    for (scale = 0.1f; scale < 1.f; scale += 0.2f) {
        scale_along_line(tempPos, cPos, sMarioCamState->pos, scale);
        tempFloorHeight = find_floor(tempPos[0], tempPos[1], tempPos[2], &tempFloor) + MARIO_EYE_HEIGHT;
    ...
    if (sMarioCamState->action & ACT_FLAG_ON_POLE) {
        camFloorHeight = gMarioStates[0].usedObj->oPosY + MARIO_EYE_HEIGHT;
    ...
    if (ceilHeight != CELL_HEIGHT_LIMIT) {
        ...
        if (c->pos[1] > ceilHeight) {
            avoidStatus = is_range_behind_surface(c->pos, sMarioCamState->pos, ceil, 0, SURFACE_NULL);
            if (avoidStatus == AVOID_STATUS_WALL_NEAR_CAMERA) {
                c->pos[1] = ceilHeight;
            }
        }
    }
...
s32 update_spiral_stairs_camera(struct Camera *c, Vec3f focus, Vec3f pos) {
    ...
    focY = sMarioCamState->pos[1] + MARIO_EYE_HEIGHT + focusHeight;
    ...
    sSpiralStairsYawOffset = CLAMP(sSpiralStairsYawOffset, DEGREES(-90), DEGREES(90));
    ...
    if (floorHeight != FLOOR_LOWER_LIMIT) {
        ...
        floorHeight += MARIO_EYE_HEIGHT;
        pos[1] = approach_f32(pos[1], floorHeight, 30.f, 30.f);
    }
...
s32 update_slide_or_0f_camera(UNUSED struct Camera *c, Vec3f focus, Vec3f pos) {
    ...
    focus_on_mario(focus, pos, MARIO_EYE_HEIGHT, MARIO_EYE_HEIGHT, 800.f, DEGREES(30), yaw);
    return sMarioCamState->faceAngle[1];
}
...
s32 update_c_up(UNUSED struct Camera *c, Vec3f focus, Vec3f pos) {
    ...
    focus_on_mario(focus, pos, MARIO_EYE_HEIGHT, MARIO_EYE_HEIGHT, 250.f, pitch, yaw);
    return sMarioCamState->faceAngle[1];
}
...
void move_mario_head_c_up(UNUSED struct Camera *c) {
    ...
    // Bound looking down to -45 degrees, and looking up to 80 degrees.
    sCUpCameraPitch = CLAMP(sCUpCameraPitch, -DEGREES(45), DEGREES(80));
...
void mode_c_up_camera(struct Camera *c) {
    ...
    if (!(gCameraMovementFlags & CAM_MOVE_STARTED_EXITING_C_UP)) {
        ...
        if (sStatusFlags & CAM_FLAG_TRANSITION_OUT_OF_C_UP) {
            ...
            vec3f_sum(c->pos, sCameraStoreCUp.pos, sMarioCamState->pos);
            vec3f_sum(c->focus, sCameraStoreCUp.focus, sMarioCamState->pos);
            // Make Mario look forward
            camera_approach_s16_symmetric_bool(&sMarioCamState->headRotation[0], 0, 0x400);
            camera_approach_s16_symmetric_bool(&sMarioCamState->headRotation[1], 0, 0x400);
        } else {
...
s32 update_in_cannon(UNUSED struct Camera *c, Vec3f focus, Vec3f pos) {
    focus_on_mario(pos, focus, MARIO_EYE_HEIGHT + sCannonYOffset, MARIO_EYE_HEIGHT, 800.f,
...
void init_camera(struct Camera *c) {
    ...
    c->cutscene = CUTSCENE_NONE;
    vec3f_set(marioOffset, 0.0f, MARIO_EYE_HEIGHT, 400.0f);
...
void object_pos_to_vec3f(Vec3f dst, struct Object *obj) {
    vec3f_copy(dst, &obj->oPosVec);
}

void vec3f_to_object_pos(struct Object *obj, Vec3f src) {
    vec3f_copy(&obj->oPosVec, src);
}
 ...
 */
s32 reduce_by_dist_from_camera(s16 value, f32 maxDist, f32 posX, f32 posY, f32 posZ) {
    Vec3f pos;
    f32 dist;
    s16 pitch, yaw;
    ...
    dist = sqrtf(sqr(goalDX) + sqr(goalDY) + sqr(goalDZ));
    if (maxDist > dist) {
        pos[0] = posX;
        pos[1] = posY;
        pos[2] = posZ;
...
s32 next_lakitu_state(Vec3f newPos, Vec3f newFoc, Vec3f curPos, Vec3f curFoc,
    ...
    if (sStatusFlags & CAM_FLAG_START_TRANSITION) {
        ...
        vec3f_get_dist_and_angle(curFoc, startPos, &sModeTransition.posDist,
    ...
    if (sModeTransition.framesLeft > 0) {
        ...
        vec3f_set_dist_and_angle(curFoc, nextFoc, sModeTransition.focDist,
                                                  sModeTransition.focPitch,
                                                  sModeTransition.focYaw);
...
s32 set_camera_mode_fixed(struct Camera *c, s16 x, s16 y, s16 z) {
    ...
    if (!vec3f_compare_f32(sFixedModeBasePosition, posX, posY, posZ)) {
        basePosSet = TRUE;
        sStatusFlags &= ~CAM_FLAG_SMOOTH_MOVEMENT;
    }
    ...
    if (c->mode != CAMERA_MODE_FIXED) {
        ...
        vec3f_set(c->pos, sFixedModeBasePosition[0],
...
void parallel_tracking_init(struct Camera *c, struct ParallelTrackingPoint *path) {
    if (c->mode != CAMERA_MODE_PARALLEL_TRACKING) {
        sParTrackPath = path;
        sParTrackIndex = 0;
        vec3_zero(sParTrackTransOff.pos);
...
void cam_castle_lobby_entrance(UNUSED struct Camera *c) {
    Vec3f pos = { -813.0f, 378.0f, 1103.0f };
    vec3f_diff(sCastleEntranceOffset, pos, sFixedModeBasePosition);
}
...
void cam_bbh_room_7_mr_i_to_coffins_transition(struct Camera *c) {
    if (set_camera_mode_fixed(c, 1371, 360, -1302)) {
        transition_next_state(c, 20);
    }
}

void cam_bbh_room_7_coffins_to_mr_i_transition(struct Camera *c) {
    if (set_camera_mode_fixed(c, 2115, 260, -772)) {
        transition_next_state(c, 20);
    }
}
...
void cam_bbh_room_2_library_to_trapdoor_transition(struct Camera *c) {
    if (set_camera_mode_fixed(c, 3493, 440, 617)) {
        transition_next_state(c, 20);
    }
}
...
void cam_bbh_room_2_trapdoor_transition(struct Camera *c) {
    if (set_camera_mode_fixed(c, 3502, 440, 1217)) {
        transition_next_state(c, 20);
    }
}
...
void cam_bbh_room_9_attic_transition(struct Camera *c) {
    if (set_camera_mode_fixed(c, -670, 460, 372)) {
        transition_next_state(c, 20);
    }
}

void cam_bbh_room_9_mr_i_transition(struct Camera *c) {
    if (set_camera_mode_fixed(c, 131, 380, -263)) {
        transition_next_state(c, 20);
    }
}
...
void resolve_geometry_collisions(Vec3f pos) {
    ...
    s32 hasFloor = (floorY != FLOOR_LOWER_LIMIT);
    s32 hasCeil  = ( ceilY != CELL_HEIGHT_LIMIT);

    if (hasFloor && !hasCeil) {
        floorY += MARIO_EYE_HEIGHT;
        if (pos[1] < floorY) {
            pos[1] = floorY;
        }
    }
    ...
    if (hasFloor && hasCeil) {
        ...
        s32 underFloor = (pos[1] <= floorY);
        s32 aboveCeil  = (pos[1] >=  ceilY);

        if (underFloor && !aboveCeil) {
            pos[1] = floorY;
        }
        if (!underFloor && aboveCeil) {
            pos[1] = ceilY;
 ...
 *
 * @return AVOID_STATUS_WALL_COVERING_MARIO if a wall is covering Mario,
 *         AVOID_STATUS_WALL_NEAR_CAMERA if a wall is only near the camera.
 */
s32 rotate_camera_around_walls(UNUSED struct Camera *c, Vec3f cPos, s16 *avoidYaw, s16 yawRange) {
    ...
    s16 yawFromMario = atan2s(marioToCamera[2], marioToCamera[0]);
    ...
    for (step = 0; step < CAMERA_NUM_OBSTRUCTION_CHECKS; step++) {
        ...
        if (find_wall_collisions(&colData) != 0) {
            ...
            if (step > (CAMERA_NUM_OBSTRUCTION_CHECKS / 2)) {
                sStatusFlags |= CAM_FLAG_CAM_NEAR_WALL;
                if (status <= AVOID_STATUS_NONE) {
                    status = AVOID_STATUS_WALL_NEAR_CAMERA;
                    wall = colData.walls[colData.numWalls - 1];
                    // Calculate the avoid direction.
            ...
            if (!is_range_behind_surface(sMarioCamState->pos, cPos, wall, yawRange, SURFACE_WALL_MISC)
                ...
                && !is_surf_within_bounding_box(wall, -1.0f, (MARIO_HITBOX_HEIGHT - 10.0f), -1.0f)) {
                // Calculate the avoid direction.
                *avoidYaw = calc_avoid_yaw(yawFromMario, wallYaw);
                camera_approach_s16_symmetric_bool(avoidYaw, wallYaw, yawRange);
                status = AVOID_STATUS_WALL_COVERING_MARIO;
...
void cutscene_ending_peach_appear_closeup(struct Camera *c) {
    ...
    vec3f_copy_y_off(c->focus, &gCutsceneFocus->oPosVec, 125.0f);
}
...
void cutscene_ending_peach_descends(struct Camera *c) {
    ...
    vec3f_copy_y_off(c->focus, &gCutsceneFocus->oPosVec, sCutsceneVars[2].point[1]);
    player2_rotate_cam(c, -0x2000, 0x2000, -0x2000, 0x2000);
}
...
void cutscene_dance_closeup_focus_mario(struct Camera *c) {
    ...
    vec3f_copy_y_off(marioPos, sMarioCamState->pos, MARIO_EYE_HEIGHT);
...
void cutscene_bowser_arena_set_pos(struct Camera *c) {
    vec3f_set_dist_and_angle(sCutsceneVars[2].point, c->pos, sCutsceneVars[3].point[2],
                                                             sCutsceneVars[3].angle[0],
                                                             sCutsceneVars[3].angle[1]);
    vec3f_copy(sCutsceneVars[2].point, &gSecondCameraFocus->oPosVec);
}
...
void cutscene_bowser_arena_start(struct Camera *c) {
    ...
    vec3i_to_vec3s(sCutsceneVars[2].angle, &gSecondCameraFocus->oMoveAngleVec);
...
void cutscene_exit_to_castle_grounds_focus_mario(struct Camera *c) {
    vec3f_copy(c->focus, sMarioCamState->pos);
    c->focus[1] = c->pos[1] + (sMarioCamState->pos[1] + MARIO_EYE_HEIGHT - c->pos[1]) * 0.5f;
...
void cutscene_goto_cvar_pos(struct Camera *c, f32 goalDist, s16 goalPitch, s16 rotPitch, s16 rotYaw) {
    ...
    nextPitch = CLAMP(nextPitch, -0x3000, 0x3000);
...
void cutscene_dialog_start(struct Camera *c) {
    ...
    sCutsceneVars[8].point[1] += MARIO_EYE_HEIGHT;
...
void cutscene_non_painting_set_cam_pos(struct Camera *c) {
    ...
    switch (gPrevLevel) {
        ...
        default:
            ...
            c->pos[1] = find_floor(c->pos[0], c->pos[1] + 1000.f, c->pos[2], &floor) + MARIO_EYE_HEIGHT;
            break;
    }
}
...
void cutscene_exit_bowser_succ(struct Camera *c) {
    ...
    cutscene_event(cutscene_exit_bowser_key_toss_shake,  c, MARIO_EYE_HEIGHT, MARIO_EYE_HEIGHT);
...
void cutscene_non_painting_end(struct Camera *c) {
    ...
    sStatusFlags |= (CAM_FLAG_UNUSED_CUTSCENE_ACTIVE | CAM_FLAG_SMOOTH_MOVEMENT);
...
/**
 * Approach a y offset of MARIO_EYE_HEIGHT again.
 */
void cutscene_sliding_doors_fly_back_up(UNUSED struct Camera *c) {
    camera_approach_f32_symmetric_bool(&sCutsceneVars[0].point[1], MARIO_EYE_HEIGHT, 10.f);
}
...
void cutscene_sliding_doors_follow_mario(struct Camera *c) {
    ...
    camera_approach_f32_symmetric_bool(&sCutsceneVars[0].point[2], MARIO_EYE_HEIGHT, 50.f);
    ...
    set_focus_rel_mario(c, 0, MARIO_EYE_HEIGHT, 0, 0);
}
...
void cutscene_enter_painting(struct Camera *c) {
    ...
    if (ripplingPainting != NULL) {
        ...
        f32 size = ((painting->sizeX + painting->sizeY) / 2.0f);
...
void cutscene_exit_painting_move_to_floor(struct Camera *c) {
    ...
    if (floor != NULL) {
        floorHeight[1] = (floorHeight[1] + ((sMarioCamState->pos[1] - floorHeight[1]) * 0.7f) + MARIO_EYE_HEIGHT);
...
void cutscene_unused_exit_start(struct Camera *c) {
    ...
    set_focus_rel_mario(c, 0.f, MARIO_EYE_HEIGHT, 0.f, 0);
}
...
void cutscene_unused_exit_focus_mario(struct Camera *c) {
    ...
    vec3f_copy_y_off(focus, sMarioCamState->pos, MARIO_EYE_HEIGHT);
    set_focus_rel_mario(c, 0.f, MARIO_EYE_HEIGHT, 0.f, 0);
...
void cutscene_enter_cannon_start(struct Camera *c) {
    struct Object *obj = sMarioCamState->usedObj;
...
void cutscene_door_move_behind_mario(struct Camera *c) {
    ...
    vec3f_set(camOffset, 0.f, MARIO_EYE_HEIGHT, 250.f);
