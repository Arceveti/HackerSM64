...
s32 should_begin_sliding(struct MarioState *m) {
    if (m->input & INPUT_ABOVE_SLIDE) {
        _Bool superSlippery = ((m->floor != NULL) && (m->floor->type == SURFACE_SUPER_SLIPPERY));
        _Bool slideLevel = ((m->area->terrainType & TERRAIN_MASK) == TERRAIN_SLIDE);
        _Bool movingBackward = (m->forwardVel <= -1.0f);
...
s32 act_hold_walking(struct MarioState *m) {
    if (obj_has_behavior(m->heldObj, bhvJumpingBox)) {
