...
void exec_anim_sound_state(struct SoundState* soundStates) {
    ...
    if (soundStates[stateIdx].playSound) {
        ...
        animFrame = soundStates[stateIdx].animFrame1;
        if (animFrame >= 0) {
        ...
        animFrame = soundStates[stateIdx].animFrame2;
        if (animFrame >= 0) {
            if (cur_obj_check_anim_frame(animFrame)) {
                cur_obj_play_sound_2(soundStates[stateIdx].soundMagic);
            }
 ...
 */
UNUSED s32 calc_dist_to_volume_range_1(f32 distance) { // range from 60-124
    s32 volume;
...
UNUSED s32 calc_dist_to_volume_range_2(f32 distance) { // range from 79.2-143.2
    s32 volume;
