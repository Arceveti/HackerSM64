...
Gfx* geo_update_layer_transparency(s32 callContext, struct GraphNode* node, UNUSED void* context) {
    ...
    if (callContext == GEO_CONTEXT_RENDER) {
        ...
        if (objectOpacity == 0xFF) {
            ...
            if (
                parameter != GEO_TRANSPARENCY_MODE_NO_DITHER &&
...
s32 cur_obj_rotate_yaw_toward(s16 target, s16 increment) {
    ...
    o->oAngleVelYaw = (s16)((s16)o->oMoveAngleYaw - startYaw);

    return (o->oAngleVelYaw == 0);
}
...
UNUSED struct Object* obj_nearest_object_with_behavior_and_condition(struct Object* curObj, const BehaviorScript* behavior, s32 condition(struct Object* obj)) {
    ...
    while (obj != (struct Object*)listHead) {
        if (
            ...
            f32 objDist = dist_between_objects_squared(curObj, obj);
            if (objDist < minDist) {
                closestObj = obj;
                minDist = objDist;
            }
...
void cur_obj_get_thrown_or_placed(f32 forwardVel, f32 velY, s32 thrownAction) {
    if (cur_obj_has_behavior(bhvBowser)) {
...
static f32 cur_obj_move_y_and_get_water_level(f32 gravity, f32 buoyancy) {
    ...
    return find_water_level(o->oPosX, o->oPosY, o->oPosZ);
}
...
static s32 cur_obj_detect_steep_floor(s16 steepAngleDegrees) {
    ...
    if (o->oForwardVel != 0.0f) {
        ...
        if (intendedFloorHeight < FLOOR_LOWER_LIMIT_MISC) {
            ...
            (intendedFloor->normal.y < coss(degrees_to_angle(steepAngleDegrees))) &&
            (deltaFloorHeight > 0) &&
...
s32 cur_obj_is_mario_in_room(void) {
    if (o->oRoom != -1 && gMarioCurrentRoom != 0) {
        if (
            gMarioCurrentRoom == o->oRoom // Object is in Mario's room.
...
UNUSED s32 mario_is_within_rectangle(s16 minX, s16 maxX, s16 minZ, s16 maxZ) {
    return !(
        gMarioObject->oPosX < minX || maxX < gMarioObject->oPosX ||
...
s32 player_performed_grab_escape_action(void) {
    ...
    return ((gMarioState->controller->buttonPressed & (A_BUTTON | B_BUTTON | Z_TRIG)) != 0);
}
