...
static struct TTCPitBlockProperties sTTCPitBlockProperties[][2] = {
    [TTC_SPEED_SLOW   ] = { { .speed = 11, .waitTime = 20 }, { .speed =  -9, .waitTime = 30 } },
    [TTC_SPEED_FAST   ] = { { .speed = 18, .waitTime = 15 }, { .speed = -11, .waitTime = 15 } },
    [TTC_SPEED_RANDOM ] = { { .speed = 11, .waitTime = 20 }, { .speed =  -9, .waitTime = -1 } },
    [TTC_SPEED_STOPPED] = { { .speed =  0, .waitTime =  0 }, { .speed =   0, .waitTime =  0 } },
};
...
void bhv_ttc_pit_block_update(void) {
    if (o->oTimer > o->oTTCPitBlockWaitTime) {
        ...
        if (clamp_f32(&o->oPosY, o->oHomeY, o->oTTCPitBlockPeakY)) {
            ...
            o->oTTCPitBlockWaitTime = sTTCPitBlockProperties[gTTCSpeedSetting][o->oTTCPitBlockDir & 0x1].waitTime;

            if (o->oTTCPitBlockWaitTime < 0) {
