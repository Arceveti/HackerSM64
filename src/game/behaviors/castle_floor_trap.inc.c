...
void bhv_castle_floor_trap_open_detect(void) {
    ...
    if (action == ACT_SPECIAL_EXIT_AIRBORNE
     || action == ACT_SPECIAL_DEATH_EXIT) {
        ...
        if (o->oInteractStatus & INT_STATUS_TRAP_TURN) {
            o->oAction = CASTLE_FLOOR_TRAP_ACT_OPEN; // detects interact then opens the trapdoor
        }
    }
}
...
void bhv_castle_floor_trap_close_detect(void) {
    if (o->oDistanceToMario > 1000.0f) {
        o->oAction = CASTLE_FLOOR_TRAP_ACT_CLOSE; // close trapdoor
    }
}
...
void bhv_castle_floor_trap_loop(void) {
    switch (o->oAction) {
        case CASTLE_FLOOR_TRAP_ACT_OPEN_DETECT:
            bhv_castle_floor_trap_open_detect();
            break;
        case CASTLE_FLOOR_TRAP_ACT_OPEN:
            bhv_castle_floor_trap_open();
            break;
        case CASTLE_FLOOR_TRAP_ACT_CLOSE_DETECT:
            bhv_castle_floor_trap_close_detect();
            break;
        case CASTLE_FLOOR_TRAP_ACT_CLOSE:
            bhv_castle_floor_trap_close();
            break;
        case CASTLE_FLOOR_TRAP_ACT_ROTATE:
