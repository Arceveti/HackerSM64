...
void bhv_goomba_triplet_spawner_update(void) {
    ...
    if (o->oAction == GOOMBA_TRIPLET_SPAWNER_ACT_UNLOADED) {
        if (o->oDistanceToMario < 3000.0f) {
            ...
            s32 dAngle =
                0x10000
                / (((o->oBehParams2ndByte & GOOMBA_BP_TRIPLET_FLAG_MASK) >> 2) + 3);

            for (angle = 0x0, goombaFlag = BIT(8); angle < 0xFFFF; angle += dAngle, goombaFlag <<= 1) {
...
#ifdef FLOOMBAS
                    spawn_object_relative((o->oBehParams2ndByte & GOOMBA_BP_SIZE_MASK) | (goombaFlag >> 6),
...
static void floomba_act_startup(void) {
    ...
    if ((GET_BPARAM3(o->oBehParams) & BITMASK(7)) > o->oZoomCounter) {
        f32 frac = (f32) o->oZoomCounter / (f32) (GET_BPARAM3(o->oBehParams) & BITMASK(7));
