...
void update_terrain_objects(void) {
    ...
    DEBUG_ASSERT(((uintptr_t)gDynamicSurfacePoolEnd <= ((uintptr_t)gDynamicSurfacePool + DYNAMIC_SURFACE_POOL_SIZE)), ASSERT_PREFIX_LEVEL"Dynamic surface pool size exceeded");
}
