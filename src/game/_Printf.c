...
const char _spaces[] = "                                ";
const char _zeroes[] = "00000000000000000000000000000000";

static void _Putfld(printf_struct* printfArg, va_list* args, u8 type, unsigned char* buff);
...
s32 _Printf(char*(*prout)(char*, const char*, size_t), char* dst, const char* fmt, va_list args) {
    ...
    while (TRUE) {
        fmt_ptr = (unsigned char*)fmt;
        while ((c = *fmt_ptr) != '\0' && c != '%') {
            fmt_ptr++;
        }
        _PROUT(dst, fmt, fmt_ptr - (unsigned char*)fmt);
        if (c == '\0') {
            return printfArg.size;
        }
        fmt = (char*)++fmt_ptr;
        ...
        printfArg.width -= (
            printfArg.part1_len          +
            printfArg.num_leading_zeros  +
            printfArg.part2_len          +
            printfArg.num_mid_zeros      +
            printfArg.part3_len          +
            printfArg.num_trailing_zeros
        );
...
static void _Putfld(printf_struct* printfArg, va_list* args, u8 type, unsigned char* buff) {
    printfArg->part1_len          = 0;
    printfArg->num_leading_zeros  = 0;
    printfArg->part2_len          = 0;
    printfArg->num_mid_zeros      = 0;
    printfArg->part3_len          = 0;
