...
static struct InteractionHandler sInteractionHandlers[] = {
    ...
    { .interactType = INTERACT_WATER_RING,     .handler = interact_water_ring     },
    { .interactType = INTERACT_STAR_OR_KEY,    .handler = interact_star_or_key    },
    { .interactType = INTERACT_BBH_ENTRANCE,   .handler = interact_bbh_entrance   },
    ...
    { .interactType = INTERACT_IGLOO_BARRIER,  .handler = interact_igloo_barrier  },
    { .interactType = INTERACT_TORNADO,        .handler = interact_tornado        },
    { .interactType = INTERACT_WHIRLPOOL,      .handler = interact_whirlpool      },
    { .interactType = INTERACT_STRONG_WIND,    .handler = interact_strong_wind    },
    { .interactType = INTERACT_FLAME,          .handler = interact_flame          },
    { .interactType = INTERACT_SNUFIT_BULLET,  .handler = interact_snufit_bullet  },
    { .interactType = INTERACT_CLAM_OR_BUBBA,  .handler = interact_clam_or_bubba  },
    { .interactType = INTERACT_BULLY,          .handler = interact_bully          },
    { .interactType = INTERACT_SHOCK,          .handler = interact_shock          },
    { .interactType = INTERACT_BOUNCE_TOP2,    .handler = interact_bounce_top     },
    ...
    { .interactType = INTERACT_HOOT,           .handler = interact_hoot           },
    { .interactType = INTERACT_BREAKABLE,      .handler = interact_breakable      },
    { .interactType = INTERACT_KOOPA,          .handler = interact_bounce_top     },
    { .interactType = INTERACT_KOOPA_SHELL,    .handler = interact_koopa_shell    },
    { .interactType = INTERACT_SPINY_WALKING,  .handler = interact_spiny_walking  },
...
u32 bully_knock_back_mario(struct MarioState *mario) {
    ...
    if (marioDYaw < -DEGREES(90) || marioDYaw > DEGREES(90)) {
        mario->faceAngle[1] += DEGREES(180);
...
UNUSED static u32 unused_determine_knockback_action(struct MarioState *m) {
    ...
    if (facingDYaw >= -DEGREES(90) && facingDYaw <= DEGREES(90)) {
        m->forwardVel *= -1.0f;
...
u32 should_push_or_pull_door(struct MarioState *m, struct Object *obj) {
    ...
    return ((dYaw <= DEGREES(90)) ? WARP_FLAG_DOOR_PULLED : WARP_FLAG_DOOR_FLIP_MARIO);
}
...
u32 interact_star_or_key(struct MarioState *m, UNUSED u32 interactType, struct Object *obj) {
    ...
    if (m->health >= 0x100) {
        ...
        starIndex = ((obj->oBehParams >> 24) & BITMASK(8));
#else
        starIndex = ((obj->oBehParams >> 24) & BITMASK(5));
#endif
...
u32 interact_bbh_entrance(struct MarioState *m, UNUSED u32 interactType, struct Object *obj) {
    if (m->action != ACT_BBH_ENTER_SPIN
     && m->action != ACT_BBH_ENTER_JUMP) {
        mario_stop_riding_and_holding(m);
    ...
    if (actionId >= ACT_GROUP_AIRBORNE && actionId < (ACT_HOLD_JUMP & ACT_ID_MASK)) {
        if (!(m->prevAction & ACT_FLAG_ON_POLE) || m->usedObj != obj) {
            ...
            if (height < obj->oPosY) {
                ...
                m->marioObj->oMarioPolePos = height - obj->oPosY;
            }
...
u32 mario_can_talk(struct MarioState *m, UNUSED u32 dontRequireWall) {
    u32 action = m->action;
    ...
    if (action == ACT_WALKING
     || action == ACT_BRAKING
     || action == ACT_DECELERATING
     || action == ACT_BRAKING_STOP) {
        return TRUE;
    }

    return FALSE;
}
#else // !EASIER_DIALOG_TRIGGER
u32 mario_can_talk(struct MarioState *m, u32 dontRequireWall) {
    u32 action = m->action;

    if (action & ACT_FLAG_IDLE) {
        return TRUE;
    }
...
u32 check_read_sign(struct MarioState *m, struct Object *obj) {
    ...
    if (object_facing_mario(m, obj, SIGN_RANGE)
...
void mario_handle_special_floors(struct MarioState *m) {
    ...
    if (m->floor != NULL) {
        ...
        if (!(m->action & (ACT_FLAG_AIR | ACT_FLAG_SWIMMING)) && (floorType == SURFACE_BURNING)) {
            check_lava_boost(m);
        }
    }
}
